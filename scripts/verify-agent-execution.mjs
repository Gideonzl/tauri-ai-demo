...
assert(sshProtocol.includes('channel.request_pty(false, "xterm"'), '交互终端必须以兼容模式请求 PTY，避免服务端回执导致失败')
assert(sshProtocol.includes('ChannelMsg::Success'), '交互终端必须等待服务端确认 Shell 已启动')
assert(sshProtocol.includes('ChannelMsg::Failure'), '服务端拒绝 PTY Shell 时必须返回明确错误')
assert(sshProtocol.includes('oneshot::channel'), '打开 PTY 时必须将启动结果回传给前端')
assert(sshProtocol.includes('same_channel(&cmd_tx)'), '失败的旧 PTY 不得清空新 PTY 的输入通道')
assert(sshProtocol.includes('reconnect_session(session_id).await?'), 'PTY 打开失败时必须自动重连后再重试')
...
assert(terminalPanel.includes('MAX_PTY_RETRY_ATTEMPTS'), 'PTY 启动失败后必须有受限的自动重试')
assert(terminalPanel.includes('Retrying interactive shell automatically'), 'PTY 启动失败时必须告知用户正在自动重试')
...
assert(terminalPanel.includes('const isPanelActive'), '每个终端面板必须识别自身的会话')
assert(terminalPanel.includes('isPanelActive.value && rsid'), '隐藏终端面板不得为当前会话重复打开 PTY')
