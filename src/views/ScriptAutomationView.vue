  ...
  <div ref="scriptPageRef" class="script-page">
    ...
    <main class="script-workspace">
          ...
          <div class="script-form-head">
            ...
            <div class="script-form-actions">
              ...
              <el-dropdown class="script-editor-more-menu" trigger="click">
                <el-button class="script-editor-more-icon" size="small" circle :aria-label="t('scripts.moreActions')"><el-icon :size="14"><MoreFilled /></el-icon></el-button>
                <template #dropdown>
                  <el-dropdown-menu>
      ...
      <section v-else class="script-history-view">
        <div class="history-summary"><div><span>{{ t('scripts.historyTitle') }}</span><small>{{ t('scripts.historyHint') }}</small></div><div class="history-actions"><el-select v-model="historyTimeRange" size="small" :aria-label="t('scripts.filterTimeRange')"><el-option :label="t('scripts.timeRangeAll')" value="all" /><el-option :label="t('scripts.timeRangeDay')" value="day" /><el-option :label="t('scripts.timeRangeWeek')" value="week" /><el-option :label="t('scripts.timeRangeMonth')" value="month" /></el-select><el-select v-model="historyStatusFilter" size="small" :aria-label="t('scripts.filterStatus')"><el-option :label="t('scripts.filterAll')" value="all" /><el-option :label="t('scripts.status_success')" value="success" /><el-option :label="t('scripts.status_failed')" value="failed" /><el-option :label="t('scripts.status_skipped')" value="skipped" /></el-select><el-button class="history-failed-only" size="small" :type="historyStatusFilter === 'failed' ? 'primary' : 'default'" @click="historyStatusFilter = historyStatusFilter === 'failed' ? 'all' : 'failed'">{{ t('scripts.failedOnly') }}</el-button><el-input v-model="historyQuery" size="small" clearable :placeholder="t('scripts.historySearchPlaceholder')" /><el-button size="small" @click="exportHistory"><el-icon :size="13"><Download /></el-icon>{{ t('scripts.exportHistory') }}</el-button><el-button size="small" text class="history-clear" :disabled="!filteredLogs.length" @click="clearHistory"><el-icon :size="13"><Delete /></el-icon>{{ t('scripts.clearHistory') }}</el-button></div></div>
        <div class="script-log-list">
          <article v-for="log in filteredLogs" :key="log.id" class="script-log" :class="log.status">
            <div class="script-log-head">
              <span class="script-log-status"></span><b>{{ log.scriptName }}</b><span class="script-log-server">{{ log.serverName }}</span><span class="script-log-badge">{{ statusLabel(log.status) }}</span><span class="script-log-time">{{ formatDate(log.startedAt) }}</span><span class="script-log-duration">{{ t('scripts.duration', { time: formatDuration(log) }) }}</span>
              <div class="script-log-actions"><button v-if="log.output" type="button" :title="t('scripts.copyOutput')" @click.stop="copyLogOutput(log)"><el-icon :size="13"><DocumentCopy /></el-icon></button><button v-if="log.status === 'failed'" type="button" :title="t('scripts.analyzeFailure')" @click.stop="analyzeLogWithAI(log)"><el-icon :size="13"><MagicStick /></el-icon></button><button type="button" @click="toggleLog(log.id)"><el-icon :size="13"><ArrowUp v-if="expandedLogs.has(log.id)" /><ArrowDown v-else /></el-icon></button></div>
            </div>
            <pre v-if="expandedLogs.has(log.id)">{{ log.output || t('scripts.noOutput') }}</pre>
            <p v-else>{{ outputPreview(log.output) }}</p>
          </article>
          <OpsEmptyState v-if="!filteredLogs.length" :icon="Document" :title="historyQuery || historyStatusFilter !== 'all' || historyTimeRange !== 'all' ? t('scripts.emptyFilteredHistory') : t('scripts.emptyHistory')" :description="historyQuery || historyStatusFilter !== 'all' || historyTimeRange !== 'all' ? t('scripts.emptyFilteredHistoryHint') : t('scripts.emptyHistoryHint')">
            <template #action><el-button v-if="historyQuery || historyStatusFilter !== 'all' || historyTimeRange !== 'all'" size="small" @click="historyStatusFilter = 'all'; historyTimeRange = 'all'; historyQuery = ''">{{ t('scripts.resetFilters') }}</el-button><el-button v-else size="small" @click="activeTab = 'editor'"><el-icon :size="13"><Document /></el-icon>{{ t('scripts.goToScripts') }}</el-button></template>
          </OpsEmptyState>
        </div>
      </section>
    </main>
...
type SendOperationsAnalysisToAi = (title: string, content: string, prompt: string, serverInfo?: string) => boolean | Promise<boolean>
...
const sendOperationsAnalysisToAI = inject<SendOperationsAnalysisToAi>('sendOperationsAnalysisToAI')
...
const filteredLogs = computed(() => {
  ...
  const rangeStart = historyTimeRange.value === 'day' ? Date.now() - 24 * 60 * 60 * 1000 : historyTimeRange.value === 'week' ? Date.now() - 7 * 24 * 60 * 60 * 1000 : historyTimeRange.value === 'month' ? Date.now() - 30 * 24 * 60 * 60 * 1000 : 0
  return scriptStore.runLogs.filter(log => {
    ...
    return statusMatches && textMatches && (!rangeStart || log.startedAt >= rangeStart)
  })
})
...
function draftSignature() { return JSON.stringify({ id: draft.id || '', name: draft.name, description: draft.description, content: draft.content, tags: draft.tags }) }
...
function copyIntoDraft(script?: { id: string; name: string; description: string; content: string; tags: string[] }) {
  ...
  lastSavedDraftSignature.value = draftSignature()
}
...
function newScript() { persistDraft(true); activeScriptId.value = ''; copyIntoDraft(); activeTab.value = 'editor' }
...
function persistDraft(force = false): boolean {
  clearAutoSave()
  if (!draft.content.trim()) return false
  const signature = draftSignature()
  if (!force && signature === lastSavedDraftSignature.value) return Boolean(draft.id)
...
function scheduleAutoSave() {
  clearAutoSave()
  if (!draft.content.trim() || draftSignature() === lastSavedDraftSignature.value) return
  autoSaveTimer = window.setTimeout(() => { persistDraft() }, AUTO_SAVE_DELAY)
}
...
function runNow() {
  if (!persistDraft(true)) { ElMessage.warning(t('scripts.contentRequired')); return }
...
function formatDuration(log: ScriptRunLog) { if (!log.finishedAt) return '—'; return `${Math.max(0, (log.finishedAt - log.startedAt) / 1000).toFixed(1)}s` }
function statusLabel(status: ScriptRunStatus) { return t(`scripts.status_${status}`) }
function copyLogOutput(log: ScriptRunLog) { navigator.clipboard.writeText(log.output).then(() => ElMessage.success(t('scripts.outputCopied'))).catch(() => ElMessage.error(t('scripts.outputCopyFailed'))) }
async function analyzeLogWithAI(log: ScriptRunLog) {
  if (!sendOperationsAnalysisToAI) { ElMessage.warning(t('scripts.aiUnavailable')); return }
  const evidence = `脚本：${log.scriptName}\n状态：${statusLabel(log.status)}\n开始时间：${formatDate(log.startedAt)}\n耗时：${formatDuration(log)}\n\n输出：\n${log.output || t('scripts.noOutput')}`
  if (await sendOperationsAnalysisToAI(t('scripts.failureAnalysisTitle'), evidence, t('scripts.failureAnalysisPrompt'), log.serverName)) ElMessage.success(t('scripts.failureSentToAI'))
}
...
watch(() => [draft.name, draft.description, draft.content, draft.tags], scheduleAutoSave)

onUnmounted(() => {
  persistDraft(true)
  window.removeEventListener('aiterminal:script-ai-response', onAiScriptResponse)
})
</script>
...
.script-form-fields :deep(.el-input__wrapper) { padding: 0; border: 0 !important; border-radius: 0; background: transparent !important; box-shadow: inset 0 -1px 0 transparent !important; transition: box-shadow .16s ease; }.script-form-fields :deep(.el-input__wrapper:hover) { box-shadow: inset 0 -1px 0 $color-border !important; }.script-form-fields :deep(.el-input.is-focus .el-input__wrapper) { box-shadow: inset 0 -1px 0 $color-primary !important; }.script-form-fields :deep(.el-input__inner) { padding: 0; background: transparent !important; }.script-name-input :deep(.el-input__inner) { height: 27px; font-size: 13px; font-weight: 600; color: $color-text-primary; letter-spacing: 0; }.script-description-input :deep(.el-input__inner) { height: 23px; color: $color-text-secondary; font-size: 11px; letter-spacing: 0; }
...
@media (max-width: 560px) { .script-library-head { align-items: stretch; flex-direction: column; }.script-library-actions { justify-content: flex-start; }.script-page-head, .history-summary, .schedule-card-foot { align-items: flex-start; flex-direction: column; }.history-actions { width: 100%; flex-wrap: wrap; :deep(.el-input) { flex: 1; width: auto; } }.script-tabs { width: 100%; overflow-x: auto; }.schedule-form-grid { grid-template-columns: 1fr; }.schedule-item { flex-wrap: wrap; }.schedule-copy { min-width: calc(100% - 44px); }.script-log-head { flex-wrap: wrap; }.script-log-time { margin-left: 0; }.script-editor-main, .script-schedules-view, .script-history-view { padding: 11px; } }
