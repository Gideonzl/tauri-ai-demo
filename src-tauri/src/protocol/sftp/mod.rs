...
fn is_hidden(name: &str) -> bool { name.starts_with('.') }
...
fn parse_ls_line(line: &str, dir_path: &str) -> Option<FileEntry> {
    ...
    Some(FileEntry {
        name: name.clone(), path, file_type, size,
        modified: 0,
...
pub async fn read_dir(session_id: &str, path: &str) -> AppResult<DirectoryListing> {
    ...
    if output.contains("__ERROR__") {
        return Err(AppError::new(ErrorCode::SftpReadDirFailed, format!("Cannot read directory: {}", path)));
    }

    let entries: Vec<FileEntry> = output.lines()
    ...
    sorted.sort_by(|a, b| {
        if a.file_type != b.file_type {
            if a.file_type == FileType::Directory { std::cmp::Ordering::Less }
    ...
    Ok(DirectoryListing { path: path.into(), entries: sorted, complete: true })
}

/// Get file stat via `stat` command
pub async fn stat(session_id: &str, path: &str) -> AppResult<FileEntry> {
    let cmd = format!("stat -c '%F,%s,%a,%Y' {} 2>/dev/null || echo '__ERROR__'", path);
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;
    let output = output.trim();

    if output.contains("__ERROR__") {
        return Err(AppError::new(ErrorCode::SftpFileOpFailed, format!("Cannot stat: {}", path)));
    }
    ...
    let perms = parts.get(2).unwrap_or(&"644").parse::<u32>().unwrap_or(0o644);
    let modified = parts.get(3).unwrap_or(&"0").parse::<u64>().unwrap_or(0);

    Ok(FileEntry {
        name: name.into(), path: path.into(), file_type, size, modified,
        permissions: perms, is_hidden: is_hidden(name),
    })
}
...
pub async fn create_dir(session_id: &str, path: &str) -> AppResult<()> {
    ...
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;
    if output.contains("__ERROR__") {
        Err(AppError::new(ErrorCode::SftpFileOpFailed, format!("mkdir failed: {}", output)))
    } else { Ok(()) }
}
...
pub async fn remove(session_id: &str, path: &str, recursive: bool) -> AppResult<()> {
    ...
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;
    if output.contains("__ERROR__") {
        Err(AppError::new(ErrorCode::SftpFileOpFailed, format!("rm failed: {}", output)))
    } else { Ok(()) }
}
...
pub async fn rename(session_id: &str, old_path: &str, new_path: &str) -> AppResult<()> {
    ...
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;
    if output.contains("__ERROR__") {
        Err(AppError::new(ErrorCode::SftpFileOpFailed, format!("mv failed: {}", output)))
    } else { Ok(()) }
}

/// Upload file (write via SSH exec)
pub async fn upload_file(session_id: &str, local_path: &str, remote_path: &str) -> AppResult<TransferProgress> {
    ...
    let cmd = format!("echo '{}' | base64 -d > {} 2>&1 || echo '__ERROR__'", b64, remote_path);
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;

    if output.contains("__ERROR__") {
        Err(AppError::new(ErrorCode::SftpUploadFailed, format!("upload failed: {}", output)))
    } else {
        Ok(TransferProgress {
            transfer_id: format!("up-{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis()),
            transferred: total, total, speed: total, done: true,
        })
    }
}

/// Download file (read via SSH exec + base64)
pub async fn download_file(session_id: &str, remote_path: &str, local_path: &str) -> AppResult<TransferProgress> {
    let cmd = format!("base64 {} 2>/dev/null || echo '__ERROR__'", remote_path);
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;
    let output = output.trim();

    if output.contains("__ERROR__") {
        return Err(AppError::new(ErrorCode::SftpDownloadFailed, format!("download failed")));
    }

    use base64::Engine;
    let data = base64::engine::general_purpose::STANDARD.decode(output)
    ...
    Ok(TransferProgress {
        transfer_id: format!("dl-{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis()),
        transferred: total, total, speed: total, done: true,
    })
}
...
pub async fn chmod(session_id: &str, path: &str, mode: &str) -> AppResult<()> {
    ...
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;
    if output.contains("__ERROR__") {
        Err(AppError::new(ErrorCode::SftpFileOpFailed, format!("chmod failed: {}", output)))
    } else { Ok(()) }
}
...
pub async fn touch(session_id: &str, path: &str) -> AppResult<()> {
    ...
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;
    if output.contains("__ERROR__") {
        Err(AppError::new(ErrorCode::SftpFileOpFailed, format!("touch failed: {}", output)))
    } else { Ok(()) }
}

/// Compress via tar
pub async fn compress(session_id: &str, source_path: &str, target_path: &str, decompress: bool) -> AppResult<()> {
    let cmd = if decompress {
        format!("tar -xzf {} -C $(dirname {}) 2>&1 || echo '__ERROR__'", target_path, target_path)
    } else {
        format!("tar -czf {} {} 2>&1 || echo '__ERROR__'", target_path, source_path)
    };
    let output = crate::protocol::ssh::exec_command(session_id, &cmd).await?;
    if output.contains("__ERROR__") {
        Err(AppError::new(ErrorCode::SftpFileOpFailed, format!("compress failed: {}", output)))
    } else { Ok(()) }
}
...
pub async fn close_sftp_channel(_sftp_id: &str) -> AppResult<()> { Ok(()) }
